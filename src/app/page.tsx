...
const SERVICES = [
  { icon: 'penTool', title: 'Neuer Webauftritt',       body: 'Noch keine Website oder neu anfangen? Wir designen und entwickeln alles — Texte, Optik und Technik — damit Sie mit etwas starten, worauf Sie stolz sind.' },
  { icon: 'layout',  title: 'Redesign & Modernisierung', body: 'Website veraltet oder bringt keine Anfragen? Wir machen daraus etwas Modernes, Schnelles und Wirkungsvolles — damit Besucher zu Kunden werden.' },
  { icon: 'heart',   title: 'Wartung & Web-Branding',  body: 'Wir verschwinden nicht nach dem Launch. Updates, kleine Änderungen und ein komplettes Markenpaket — für eine überschaubare monatliche Pauschale.' },
];

const STEPS = [
  { n: '01', title: 'Das erste Gespräch',       body: 'Wir starten mit einem entspannten Call, um Ihr Unternehmen und Ihre Ziele zu verstehen. Kein Fachjargon, kein Druck.' },
  { n: '02', title: 'Wir designen & entwickeln', body: 'Sie sehen echte Designs früh und oft. Wir kümmern uns um die Technik und halten Sie immer auf dem Laufenden.' },
  { n: '03', title: 'Launch & langfristige Betreuung', body: 'Wir bringen Sie online, erklären alles in Ruhe und bleiben für spätere Updates und Fragen an Ihrer Seite.' },
];
    ...
    <div>
              ...
              <div style={{ background: 'var(--surface-card)' }}>
                <div style={{ padding: '20px 22px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ display: 'flex', gap: '12px', fontSize: '11px', color: 'var(--text-faint)' }}>
                    <span>Sortiment</span><span>Über uns</span>
                ...
                <div style={{ padding: '14px 22px 22px' }}>
                  ...
                  <div style={{ marginTop: '8px', display: 'flex', gap: '8px' }}>
                    <span style={{ background: 'var(--gold-500)', color: 'var(--green-900)', fontSize: '11px', fontWeight: 700, padding: '6px 12px', borderRadius: 'var(--radius-pill)' }}>Online bestellen</span>
                    <span style={{ border: '1px solid var(--border-strong)', color: 'var(--text-muted)', fontSize: '11px', fontWeight: 600, padding: '6px 12px', borderRadius: 'var(--radius-pill)' }}>Unsere Geschichte</span>
                  </div>
                </div>
              </div>
            </BrowserFrame>
      ...
      <Section bg="var(--surface-brand)">
        <div className="jj-process-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 0.8fr) minmax(0, 1.2fr)', gap: 'clamp(32px, 5vw, 64px)', alignItems: 'start' }}>
          <div>
            <div className="jj-eyebrow" style={{ color: 'var(--green-300)', marginBottom: '14px' }}>So läuft es ab</div>
